...
import { verifyBearer } from "@/lib/utils/cron-auth";
...
export async function GET(req: NextRequest) {
  ...
  // Constant-time, shared with the cron guard. A plain `!==` compares byte by
  // byte and bails at the first difference, so response timing leaks the token
  // prefix to anyone patient enough to measure it.
  if (!verifyBearer(req.headers, secret)) {
    return NextResponse.json(
