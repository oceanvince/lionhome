 ...
 * than inferred.
 *
 * Separate from `lib/analytics/events.ts`, which is the server-side pipeline
 * writing to the `analytics_events` table: that module imports the Supabase
 * service-role client and must never reach a client bundle. The two vocabularies
 * are disjoint by design — this one is the fine-grained funnel, that one is the
 * four milestones the admin dashboard aggregates.
 */
 ...
 * so call sites never need to guard. Currently a no-op in every environment —
 * see components/analytics/posthog-provider.tsx for why the key stays unset.
 *
