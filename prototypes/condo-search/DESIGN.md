...
**角色约定**：
- 结论徽章配色**只**用 verdict-* 三色，不复用 band-*，避免「结论」与「单维分数」语义混淆。
...
```html
<link href="https://fonts.googleapis.com/css2?family=Noto+Sans+SC:wght@300;400;500;600&family=Noto+Serif+SC:wght@400;500;600;700&display=swap" rel="stylesheet">
```
...
### 4.1 按钮（继承）
```css
.btn-primary{ @apply w-full bg-primary text-white py-4 px-6 rounded-[4px] font-medium hover:bg-primary/90 transition-colors flex items-center justify-center gap-2; min-height:56px; }
...
.btn-secondary{ @apply w-full bg-white border border-border text-charcoal py-4 px-6 rounded-[4px] font-medium hover:bg-gray-50 transition-colors; min-height:56px; }
```

### 4.2 结论徽章 `.verdict-badge`（PRD §4.1.2）
```css
.verdict-badge{ @apply inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-medium; }
.verdict-badge.is-green { color:var(--color-verdict-green);  background:var(--color-verdict-green-soft); }
.verdict-badge.is-amber { color:var(--color-verdict-amber);  background:var(--color-verdict-amber-soft); }
.verdict-badge.is-orange{ color:var(--color-verdict-orange); background:var(--color-verdict-orange-soft); }
/* 前缀圆点用 currentColor */
```
...
```css
.dim-card{ @apply bg-white border border-border rounded-[4px] p-5 transition-all; }
.dim-card:hover{ @apply border-gray-300; }
.dim-card.is-coming{ @apply bg-cream/50 border-dashed; }          /* 即将上线灰显 */
.dim-card .score{ font-family:var(--font-serif); font-size:40px; font-weight:600; font-variant-numeric:tabular-nums; line-height:1; }
.dim-card .band-pill{ @apply text-xs font-medium px-2 py-0.5 rounded-full; }
.explain-toggle{ @apply text-sm text-primary inline-flex items-center gap-1 cursor-pointer hover:underline; }  /* 「怎么算的？」 */
.explain-body{ @apply text-sm text-gray-600 leading-relaxed mt-3 pt-3 border-t border-border; }
```

### 4.4 数据诚实度标注 `.honesty-tag`（PRD §4.3，设计红线）
```css
.honesty-tag{ @apply inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-[3px]; color:var(--color-warn); background:var(--color-warn-soft); }
```
触发即显示，文案如「相似盘估算」「数据覆盖低」「新盘」。这是**信任的核心**，不可省略。

### 4.5 适配度模块 `.fit-bar`（PRD §4.1.6，与四维同级）
```css
.fit-bar{ @apply relative h-2 rounded-full bg-gray-100; }      /* 舒适区间轨道 */
.fit-bar .comfort{ @apply absolute h-full rounded-full; background:var(--color-primary-soft); }
.fit-bar .marker{ @apply absolute -top-1 w-1 h-4 rounded-full; }   /* 本盘起价位置 */
.fit-dot{ @apply inline-block w-2 h-2 rounded-full; }              /* 在预算内/略超 */
```

### 4.6 项目卡片 `.project-card`（PRD §3.2，搜索结果）
```css
.project-card{ @apply bg-white border border-border rounded-[4px] p-5 transition-all cursor-pointer; }
.project-card:hover{ @apply border-gray-300 shadow-sm; }
.project-card .psf{ font-variant-numeric:tabular-nums; }
.card-action{ @apply w-9 h-9 flex items-center justify-center rounded-full border border-border bg-white text-gray-500 hover:text-primary hover:border-primary transition-colors; }
.card-action.is-on{ @apply text-primary border-primary bg-primary-soft; }    /* 收藏/对比选中 */
```

### 4.7 Tab 条 `.report-tab`（PRD §4.4，继承 .seg 思路但下划线式）
```css
.report-tab{ @apply whitespace-nowrap px-1 py-3 text-sm font-medium text-gray-500 border-b-2 border-transparent cursor-pointer transition-colors; }
.report-tab:hover{ @apply text-charcoal; }
.report-tab.active{ @apply text-primary border-primary; }
```

### 4.8 关键指标条 `.metric`（PRD §4.1.5）
```css
.metric{ @apply flex flex-col gap-0.5; }
.metric .v{ @apply text-base font-medium text-charcoal; font-variant-numeric:tabular-nums; }
.metric .k{ @apply text-xs text-gray-500 font-light; }
```

### 4.9 市场状态标签 `.regime-tag`（PRD §4.2，带 tooltip）
```css
.regime-tag{ @apply inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border border-border text-gray-600 cursor-help; }
```
...
```css
--shadow-sm:  0 1px 2px rgba(26,28,26,.04);              /* 卡片 hover */
--shadow-md:  0 4px 16px rgba(26,28,26,.06);             /* 弹层/吸底 CTA */
--shadow-card:0 1px 3px rgba(26,28,26,.05);              /* 默认卡片几乎不投影，靠 border 分隔 */
```
...
.enter{ animation:fadeInUp .4s ease-out both; }       /* 入场：区块依次淡入上移 */
.enter-2{ animation-delay:.08s } .enter-3{ animation-delay:.16s }
```
...
5. ❌ 不把结论徽章色（verdict-*）和单维档位色（band-*）混用。
6. ❌ 不出现无法解释的黑箱分数。
